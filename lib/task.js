...
function TaskJS() {
    ...
    this._loadFile('./tasks/files');
    this._loadFile('./tasks/concat');
    this._loadFile('./tasks/log');
    this._loadFile('./tasks/inspect');
}
...
TaskJS.prototype = {
    _loadFile: function(path) {
        ...
        for (name in file) {
            this._addTask(name, file[name]);
        }
    },

    _addTask: function(name, fn) {
        var self = this;
        this._tasks[name] = fn;
