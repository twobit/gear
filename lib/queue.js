...
Queue.prototype._dispatch = function(name, options, notify, messages, done) {
    ...
    switch (task.type) {
        ...
        case 'slice':
            async.map(messages.slice(0, Array.isArray(options) ? options.length : 1), task.bind(this, options), transform.bind(this, null));
            break;
...
Queue.prototype.task = function(name, options, notify) {
    this._queue.push(this._dispatch.bind(this, name, options, notify));
    return this;
};
