...
Queue.prototype._log = function(callback, err, results, view) {
    if (view) {
        view.body.forEach(function(item) {
            console.log(item);
        });
    }
...
Queue.prototype._dispatch = function(name, options, messages, done, notify) {
    ...
    switch (task.type) {
        case 'broadcast':
            async.map(
                ...
                function(option, callback) {
                    ...
                    task.fn.call(self, option, {}, self._log.bind(self, callback));
                },
        ...
        case 'join':
            task.fn.call(self, options, messages, self._log.bind(self, function(err, results) {
                if (notify) {
                    notify(err, [results]);
                }
                done(err, [results]);
        ...
        default:
            async.map(messages, function(message, callback) {
                    task.fn.call(self, options, message, self._log.bind(self, callback));
                },
                function(err, results) {
                    if (notify) {
                        notify(err, results);
